  ...
  return (
    <div className="min-h-screen bg-[#09090a] text-zinc-100">
      <div className="flex min-h-screen">
        <aside className="hidden w-64 shrink-0 border-r border-white/[0.08] bg-[#0d0d0f] px-4 py-5 lg:flex lg:flex-col">
          ...
          <div className="mb-6 flex items-center gap-3 border-b border-white/[0.06] pb-5">
            {image ? (
              ...
              <div className="flex h-10 w-10 items-center justify-center rounded-md bg-white/[0.06] text-sm font-semibold">
            ...
            {navItems.map(({ to, label, iconKey }) => {
              ...
              return (
                <Link
                  key={to}
                  to={to}
                  className="group flex items-center gap-3 rounded-md px-2 py-2 text-sm text-zinc-500 transition hover:bg-white/[0.05] hover:text-zinc-100"
                >
                  <Icon className="h-4 w-4" />
                  <span className="min-w-0 flex-1 truncate">{label}</span>
                  {isRequests && pendingCount > 0 ? (
                    <span className="rounded-full bg-white text-[10px] font-semibold text-zinc-950 px-1.5 py-0.5">
                      {pendingCount}
                    </span>
                  ) : null}
                </Link>
          ...
          <div className="mt-auto space-y-3 border-t border-white/[0.06] pt-4">
            {apiUrl !== undefined ? (
              calendarAuthorized ? (
                ...
                <button
                  ...
                  className="w-full rounded-md px-2 py-2 text-left text-xs text-zinc-500 transition hover:bg-white/[0.05] hover:text-zinc-100"
                >
                  Connect Calendar
                </button>
              )
            ...
            <button
              onClick={logout}
              className="flex w-full items-center gap-3 rounded-md px-2 py-2 text-sm text-zinc-500 transition hover:bg-white/[0.05] hover:text-zinc-100"
            >
            ...
            <header className="flex flex-col gap-5 border-b border-white/[0.08] pb-7 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {slug ? (
                  ...
                  <Link
                    to="/lounge/profile"
                    className="inline-flex h-10 items-center rounded-md bg-white px-3 text-sm font-medium text-zinc-950 transition hover:bg-zinc-200"
                  >
                    Set up link
                  </Link>
                )}
            ...
            <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
              <div className="space-y-6">
                <div className="border-b border-white/[0.08] pb-6">
                  <div className="mb-3 flex items-center justify-between gap-4">
                    ...
                    <div className="flex shrink-0 gap-1">
                      <button
                        ...
                        title={t('common.copy')}
                        className="flex h-9 w-9 items-center justify-center rounded-md text-zinc-400 transition hover:bg-white/[0.06] hover:text-white disabled:opacity-40"
                      >
                  ...
                  <div className="flex min-h-11 items-center gap-3 rounded-md bg-white/[0.04] px-3">
                    <span className={`h-2 w-2 rounded-full ${profileLink ? 'bg-emerald-400' : 'bg-zinc-600'}`} />
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <Link
                    ...
                    className="group border-b border-white/[0.08] pb-5 transition hover:border-white/30"
                  >
                    ...
                    <div className="mt-3 flex items-end justify-between gap-4">
                      ...
                      <ArrowRight className="mb-2 h-5 w-5 text-zinc-600 transition group-hover:translate-x-1 group-hover:text-white" />
                    </div>
                ...
                <section>
                  ...
                  {nextRequest ? (
                    <Link
                      ...
                      className="group flex items-center gap-4 border-y border-white/[0.08] py-4 transition hover:border-white/30"
                    >
              ...
              <aside className="space-y-6 lg:border-l lg:border-white/[0.08] lg:pl-6">
                <section>
                  ...
                  <div className="mt-3 divide-y divide-white/[0.06] border-y border-white/[0.08]">
                    {dashboard.recentEvents.length > 0 ? (
                      dashboard.recentEvents.map((event) => (
                        ...
                        >
                          <div className="min-w-0">
                            <p className="truncate text-sm text-zinc-300">{eventLabels[event.eventType] ?? 'Meeting update'}</p>
                            <p className="mt-1 text-xs text-zinc-600">{formatDateTime(event.createdAt)}</p>
                          </div>
                          <ArrowRight className="h-4 w-4 shrink-0 text-zinc-700 transition group-hover:translate-x-1 group-hover:text-white" />
                        </Link>
                      ))
                    ) : (
