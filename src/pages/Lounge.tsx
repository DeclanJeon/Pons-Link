...
const navItems = [
  ...
  { to: '/lounge/bookings', label: 'Bookings', icon: CalendarDays },
  { to: '/lounge/email-deliveries', label: 'Email Deliveries', icon: Mail },
...
const Lounge = () => {
  ...
  const headline = profile.bootstrap.data?.publicProfile?.headline ?? 'Your personal link studio for managing conversations.';
  ...
  const stats = [
    { label: 'Friends', value: friends.list.data?.length ?? 0, icon: Users, color: 'text-sky-400', bg: 'bg-sky-500/[0.08]', border: 'border-sky-500/[0.12]' },
    { label: 'Pending', value: requests.data?.length ?? 0, icon: MessageSquareText, color: 'text-violet-400', bg: 'bg-violet-500/[0.08]', border: 'border-violet-500/[0.12]' },
    { label: 'Confirmed', value: bookings.list.data?.length ?? 0, icon: CalendarDays, color: 'text-emerald-400', bg: 'bg-emerald-500/[0.08]', border: 'border-emerald-500/[0.12]' },
  ];
      ...
      <aside className="flex w-52 shrink-0 flex-col border-r border-white/[0.07] bg-[#0C0C0C]">
        ...
        <div className="relative flex h-16 shrink-0 flex-col justify-end">
          ...
          <div className="flex items-center gap-2.5 px-4 pb-3.5">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 shadow-[0_0_10px_rgba(99,102,241,0.45)]">
        ...
        <div className="grid flex-1 grid-cols-3 grid-rows-2 gap-3 overflow-hidden p-4">
          ...
          <div className="col-span-2 flex flex-col overflow-hidden rounded-2xl border border-white/[0.07] bg-gradient-to-br from-indigo-500/[0.1] via-transparent to-transparent p-6">
            <div className="flex items-center gap-4">
              ...
              {slug && (
                <Link
                  to={`/u/${slug}`}
                  className="ml-auto inline-flex shrink-0 cursor-pointer items-center gap-1.5 rounded-full border border-white/[0.1] bg-white/[0.05] px-3 py-1.5 text-xs text-zinc-400 transition hover:bg-white/[0.09] hover:text-white"
                >
            ...
            <div className="mt-auto grid grid-cols-3 gap-3 pt-5">
              {stats.map(({ label, value, icon: Icon, color, bg, border }) => (
                <div key={label} className={`flex flex-col justify-between rounded-xl border ${border} ${bg} p-4`}>
          ...
          <div className="col-start-1 row-start-2 flex flex-col overflow-hidden rounded-2xl border border-white/[0.07] bg-[#0D0D0D] p-4">
            <div className="mb-3 flex shrink-0 items-center justify-between">
              <p className="text-[10px] uppercase tracking-[0.15em] text-zinc-700">Recent Requests</p>
            ...
            <div className="flex flex-1 flex-col justify-around gap-2 overflow-hidden">
              {requests.data && requests.data.length > 0 ? (
                requests.data.slice(0, 3).map((req: { id: string; senderName?: string; message?: string }) => (
                  <div key={req.id} className="rounded-xl border border-white/[0.06] bg-white/[0.03] px-3 py-2.5">
                    <div className="flex items-center justify-between gap-2">
                      <p className="truncate text-xs font-medium text-zinc-200">{req.senderName ?? 'Visitor'}</p>
                      <span className="shrink-0 rounded-full bg-violet-500/10 px-2 py-0.5 text-[10px] text-violet-400">pending</span>
                    </div>
                    {req.message && <p className="mt-0.5 truncate text-[10px] text-zinc-600">{req.message}</p>}
                  </div>
                ))
              ) : (
                <div className="flex flex-1 flex-col items-center justify-center text-center">
                  <MessageSquareText className="h-7 w-7 text-zinc-800" />
                  <p className="mt-2 text-xs text-zinc-700">No pending requests</p>
                </div>
              )}
            </div>
            <Link
          ...
          {/* Bookings — col 2, row 2 */}
          <div className="col-start-2 row-start-2 flex flex-col overflow-hidden rounded-2xl border border-white/[0.07] bg-[#0D0D0D] p-4">
            <div className="mb-3 flex shrink-0 items-center justify-between">
              <p className="text-[10px] uppercase tracking-[0.15em] text-zinc-700">Bookings</p>
              <CalendarDays className="h-3.5 w-3.5 text-emerald-500" />
            </div>
            <div className="flex flex-1 flex-col justify-around gap-2 overflow-hidden">
              {bookings.list.data && bookings.list.data.length > 0 ? (
                bookings.list.data.slice(0, 3).map((bk: { id: string; scheduledAt?: string; guestName?: string }) => (
                  <div key={bk.id} className="rounded-xl border border-white/[0.06] bg-white/[0.03] px-3 py-2.5">
                    <div className="flex items-center justify-between gap-2">
                      <p className="truncate text-xs font-medium text-zinc-200">{bk.guestName ?? 'Guest'}</p>
                      <span className="shrink-0 rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] text-emerald-400">confirmed</span>
                    </div>
                    {bk.scheduledAt && (
                      <p className="mt-0.5 text-[10px] text-zinc-600">
                        {new Date(bk.scheduledAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                      </p>
                    )}
                  </div>
                ))
                ...
                <div className="flex flex-1 flex-col items-center justify-center text-center">
                  ...
                  <p className="mt-2 text-xs text-zinc-700">No confirmed bookings</p>
                </div>
              )}
            </div>
            <Link
