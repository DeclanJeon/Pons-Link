...
### 최근 반영 포인트 (`main2`, v1.11.x)

- 회의실 진입/재연결 흐름 안정화
- 채팅 기반 회의록 동의 플로우 정리
- 실시간 기능별 데이터 채널 분리로 동시 전송 혼잡 완화
- STT 기본 경로를 백엔드 프록시 우선으로 정렬

### 스크린샷 기반 기능 미리보기

아래는 현재 동작 기준 화면을 기준으로 정리한 핵심 기능 캡처입니다. 각 한 줄은 해당 화면에서 실제 동작 흐름을 확인한 설명입니다.

### 🎬 진입/브랜딩

![PonsLink 랜딩과 요청 게이트 흐름](docs/readme-screenshots/01-marketing-home.png)

- **요청 중심 진입:** 홈 랜딩에서 개인 링크/요청 게이트와 영상 기반 방 진입 흐름을 한 번에 확인할 수 있습니다.

### 🛎️ 방 입장 준비

![비디오 방 로비 준비 화면](docs/readme-screenshots/02-lobby-video.png)

- **미디어 초기화 + 닉네임 설정:** 비디오 로비에서 장치 설정 후 입장 버튼으로 실시간 회의 단계로 바로 전환됩니다.

![오디오 전용 로비 준비 화면](docs/readme-screenshots/03-lobby-audio.png)

- **오디오 우선 모드 지원:** 카메라 UI는 숨기고 음성 중심 입장으로 이동해 전력/개인정보 부담을 줄입니다.

### 🎥 회의실 본체

![기본 영상 회의 인터페이스](docs/readme-screenshots/04-room-video-main.png)

- **실시간 미디어 협업 시작점:** 캠/마이크 상태와 참가자 화면 구성이 함께 표시되는 회의실 기본 동작 화면입니다.

![채팅 패널](docs/readme-screenshots/05-room-chat.png)

- **채팅 동기화:** 메시지 입력 및 동작 표시(알림/탐색)가 회의 UX 안에서 즉시 동작합니다.

![화이트보드 패널](docs/readme-screenshots/06-room-whiteboard.png)

- **동시 협업 드로잉:** 회의 중 즉석 메모·도식 작성으로 아이디어 공유 흐름을 강화합니다.

![코와치(CoWatch) 패널](docs/readme-screenshots/07-room-cowatch.png)

- **동시 콘텐츠 동기화:** 재생/시청 동기화를 함께 진행해 대화와 미디어를 같은 타임라인으로 묶습니다.

![PonsCast 패널](docs/readme-screenshots/08-room-ponscast.png)

- **방 내 미디어 스트리밍:** 방 내부에서 영상·문서 스트림을 공유하고 이어받아 확인할 수 있습니다.

![미디어 릴레이 패널](docs/readme-screenshots/09-room-relay.png)

- **미디어 경로 제어:** 세션 품질이나 환경이 바뀔 때 미디어 흐름을 관리하는 동작을 확인할 수 있습니다.
